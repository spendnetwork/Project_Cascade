...
# from core_run_files import setup, data_matching, convert_training, data_processing, data_analysis
...
def main(regiondir, in_args, directories, config_path, settings, runfilemods):
    ...
    runfilemods.setup.setupRawDirs(regiondir, directories)
    ...
    try:
        ...
        for conf_file in config_path.glob(pyfiles):
            with open(conf_file) as config_file:
                ...
                privdf = runfilemods.data_processing.clean_private_data(regiondir, directories, in_args)
                if not in_args.recycle:
                    runfilemods.data_processing.clean_public_data(regiondir, directories, in_args)
                ...
                for proc_type in configs['processes']:
                    ...
                    if in_args.recycle == configs['processes'][proc_type][main_proc]['recycle_phase']:
                        ...
                        runfilemods.setup.setup_dirs(directories, regiondir, proc_type)
                        ...
                        for proc_num in configs['processes'][proc_type]:
                            ...
                            if not os.path.exists(directories["cluster_output_file"].format(regiondir, proc_type)):
                                ...
                                runfilemods.data_matching.dedupe_match_cluster(priv_file, pub_file, regiondir, directories, configs, proc_type, proc_num, in_args)

                            if not os.path.exists(directories['assigned_output_file'].format(regiondir, proc_type)):
                                clust_df = pd.read_csv(directories["cluster_output_file"].format(regiondir, proc_type),index_col=None, dtype=df_dtypes)
                            ...
                            extracts_file = runfilemods.data_matching.extract_matches(regiondir, clust_df, configs, directories, proc_num,
                                                                          proc_type,
                        ...
                        continue
                # Output stats file:
                stat_file = runfilemods.data_analysis.calc_matching_stats(regiondir, clust_df, extracts_file, directories, conf_file_num,
    ...
    for proc_type in configs['processes']:
        ...
        if in_args.recycle == main_proc['recycle_phase']:
            ...
            runfilemods.data_matching.manual_matching(settings.regiondir, directories, best_config, proc_type, in_args)
            ...
            if in_args.convert_training:
                ...
                runfilemods.convert_training.convert_to_training(regiondir, directories, conv_file)

            if in_args.upload_to_db:
                # Add confirmed matches to relevant table
                runfilemods.db_calls.add_data_to_table(regiondir, main_proc['db_table'], directories, proc_type, in_args, settings)
...
if __name__ == '__main__':
    ...
    #     checkDataExists(settings.regiondir, settings.directories, settings.in_args, settings.publicTableSource)
    # pdb.set_trace()
    main(settings.regiondir, settings.in_args, settings.directories, settings.config_path, settings, settings.runfilemods)
