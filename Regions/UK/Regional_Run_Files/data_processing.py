...
class AssignRegDataToClusters:
    ...
    def assign(self):
        ...
        df = self.df.groupby(['Cluster ID']).progress_apply(self.get_max_id)
...
#
#         df[adj_col].replace(org_suffixes.org_suffixes_dict, regex=True, inplace=True)
#         df['src_name_short'] = df.src_name_adj.apply(shorten_name)
#
#         print("...done")
#         df.to_csv(adj_data, index=False)
#     else:
...
# def cleanMatchedData(directories, region_dir, proc_type):
