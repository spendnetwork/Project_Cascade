...
class StatsCalculations(Main):
    ...
    def calculate(self):
        ...
        statdf.at[self.conf_file_num, 'Total_Matches'] = len(clustdf[pd.notnull(clustdf['reg_id'])])
        statdf.at[self.conf_file_num, 'Percent_Matches'] = round(len(clustdf[pd.notnull(clustdf['reg_id'])]) / len(srcdf) * 100,2)
