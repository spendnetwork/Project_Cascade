...
def calc_matching_stats(rootdir, clustdf, extractdf, config_dirs, conf_file_num, proc_type, privdf):
    ...
    if os.path.exists(config_dirs['stats_file'].format(rootdir, proc_type)):
        if conf_file_num == 1:
            os.remove(config_dirs['stats_file'].format(rootdir, proc_type))
    ...
    # if it does exist, concat current results with previous
        ...
        main_stat_file = pd.read_csv(config_dirs['stats_file'].format(rootdir, proc_type), index_col=None)
        main_stat_file = pd.concat([main_stat_file, statdf], ignore_index=True, sort=True)
        main_stat_file.to_csv(config_dirs['stats_file'].format(rootdir, proc_type), index=False,
