...
def convert_textfiles(input_dir):

    # Remove all csvs from input_dir
    [os.remove(os.path.join(input_dir, f)) for f in os.listdir(input_dir) if f.endswith('.csv')]

    # Unzip all gzip files in input_dir
    # [gzip.decompress(f) for f in os.listdir(input_dir) if f.endswith('.gz')]
    subprocess.Popen([f'gunzip {input_dir}/*'], cwd='.', shell=True)
    # For each file in folder
    for f in os.listdir(input_dir):
        if f.endswith('.txt'):
            # Obtain classification and source from file name
            file_class, file_source = f.split('-')
            file_source = os.path.splitext(file_source)[0]

            with open(os.path.join(input_dir, f), 'r') as file:
                lines = file.readlines()
                # create ('+') blank csv file for appending ('a') to
                with open(os.path.join(input_dir, file_class + '.csv'), 'a+') as new_file:
                    writer = csv.writer(new_file, delimiter=',')
                    for line in lines:
                        line = [line.strip().upper(), file_source.upper()]
                        writer.writerow([t for t in line])


    for f in os.listdir(input_dir):
        if f.endswith('.csv'):
            df = pd.read_csv(os.path.join(input_dir, f))
            df.columns = ['org_string','source']
            df['org_string'] = df['org_string'].str.replace("'", '')
            df['org_string'] = df['org_string'].str.replace('"', '')
            df = df.drop_duplicates(['org_string'])
            df.to_csv(os.path.join(input_dir, f), index= False)
...
def import_data(input_dir, file_encoding='utf8', ):
    ...
    for f in os.listdir(input_dir):
        if f.endswith('.csv'):
            file_d = pd.read_csv(os.path.join(input_dir, f), encoding=file_encoding, usecols=['org_string'])
            file_d.columns = [NAME_FIELD]
            file_d.loc[:, CLASS_FIELD] = f.replace(".txt", '').replace('.gz', '').replace('.csv','')
...
def save_model(model, output_file):
    ...
    if output_file.endswith(".gz"):
        a = gzip.open(output_file, 'wb')
    else:
        a = open(output_file, 'wb')
    with a:
...
def main():
    ...
    # files to use
    parser.add_argument('--input-dir', default='updated_model_inputs', help='Location of directory containing .txt files to help classification. The files should contain a list of strings to be classified, while the filename gives the classification category.')
    ...
    convert_textfiles(args.input_dir)
