...
def get_input_args():
	...
	parser.add_argument('--priv_raw_name', default='private_data.csv', type=str, help='Set raw private/source datafile name')
...
def clean_private_data(config_dirs):
	...
	if not os.path.exists(adj_data):
		...
		df['priv_name_adj'] = df['priv_name'].str.translate(str.maketrans({key: None for key in string.punctuation})).str.replace("  ", " ").str.lower().str.strip()
...
def clean_public_data(config_dirs):
	...
	if not os.path.exists(adj_data):
		...
		for chunk in df:
			# Remove punctuation and double spacing
			chunk['pub_name_adj'] = chunk['org_name'].str.translate(str.maketrans({key: None for key in string.punctuation})).str.replace("  ", " ").str.lower().str.strip()
...
def extract_matches(clustdf, configs, config_dirs, proc_num, proc_type, conf_file_num):
	...
	if not os.path.exists(config_dirs['extract_matches_file'].format(proc_type) + '_' + str(conf_file_num) + '.csv'):
		clustdf.to_csv(config_dirs['extract_matches_file'].format(proc_type) + '_' + str(conf_file_num) + '.csv', index=False)
		return clustdf
	else:
		extracts_file = pd.read_csv(config_dirs['extract_matches_file'].format(proc_type) + '_' + str(conf_file_num) + '.csv', index_col=None)
		...
		extracts_file.to_csv(config_dirs['extract_matches_file'].format(proc_type) + '_' + str(conf_file_num) + '.csv', index=False)
...
def calc_matching_stats(clustdf, extractdf, config_dirs, conf_file_num, proc_type):
	...
	# if it does exist, concat current results with previous
		...
		main_stat_file.to_csv(config_dirs['stats_file'].format(proc_type), index=False, columns=['Config_File', 'Leven_Dist_Avg','Optim_Matches','Percent_Matches','Percent_Precision','Percent_Recall','Total_Matches'])
					...
					if in_args.recycle == configs['processes'][proc_type][min(configs['processes'][proc_type].keys())]['recycle_phase']:
						...
						for proc_num in configs['processes'][proc_type]:
							...
							clustdtype = {'Cluster ID':np.int, 'Confidence Score': np.float,
												...
												'pub_address':np.str, 'priv_name_short':np.str, 'pub_name_short':np.str, 'leven_dist':np.int}
							...
							if not os.path.exists(config_dirs['assigned_output_file'].format(proc_type)):
								...
								clustdf = pd.read_csv(config_dirs["assigned_output_file"].format(proc_type), index_col=None, dtype=clustdtype, usecols=clustdtype.keys())
